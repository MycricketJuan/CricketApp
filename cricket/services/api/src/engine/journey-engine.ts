...
export class JourneyEngine {
  ...
  async process(
    ...
    const intent = await this.classifyIntent(message.content, session, tenant)
    ...
    try {
      output = await this.runAgent(targetStage, message.content, context, tenant)
    } catch (err: unknown) {
...
Mensaje: "${message}"`,
  ...
  // ── Stage Resolver ────────────────────────────────────────
  // Determina a qué agente enrutar. No es un flujo lineal:
  // desde cualquier etapa se puede saltar a cualquier otra.
  ...
  // ── Agent Runner ──────────────────────────────────────────
  // Invoca el agente Claude con el prompt especializado
  // para la etapa (consultation, sales, transactions, feedback).
  ...
  private async runAgent(
    ...
    const { AgentRegistry } = await import('@cricket/agents/registry')
    const registry = new AgentRegistry(this.client, tenant.claude_config)
    return registry.run(stage, message, context)
  }
